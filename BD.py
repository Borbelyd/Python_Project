...
class BD_TypingTestApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Gyorsíró Teszt - BD")

        self.target_text = ""
        self.start_time = None
        self.best_time = self.BD_load_best_time()
        ...
        self.entry_text.bind("<KeyRelease>", self.BD_check_input)

        self.label_feedback = tk.Label(self.root, text="", font=("Times New Roman", 14))
        self.label_feedback.pack(pady=5)

        self.button_start = tk.Button(self.root, text="Kezdés", font=("Times New Roman", 14), command=self.BD_start_test)
        self.button_start.pack(pady=10)

        self.label_result = tk.Label(self.root, text="", font=("Times New Roman", 14))
        self.label_result.pack(pady=10)

        self.button_retry = tk.Button(self.root, text="Új teszt", font=("Times New Roman", 14), command=self.BD_reset_test, state=tk.DISABLED)
        ...
        self.BD_update_best_time_label()

    def run(self):
        self.root.mainloop()

    def BD_generate_random_text(self, length=10):
        characters = string.ascii_letters + string.digits
        return ''.join(random.choice(characters) for _ in range(length))

    def BD_start_test(self):
        self.target_text = self.BD_generate_random_text()
    ...
    def BD_check_input(self, event):
        ...
        if current_text == self.target_text:
            ...
            if self.best_time is None or elapsed_time < self.best_time:
                ...
                self.BD_save_best_time(elapsed_time)
                self.BD_update_best_time_label()
    ...
    def BD_reset_test(self):
        self.BD_start_test()

    def BD_update_best_time_label(self):
    ...
    def BD_save_best_time(self, best_time):
